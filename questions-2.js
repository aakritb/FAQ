window.ASSUREONE_ARTICLES.push(...[
  ...
  },
  {
    ...
    "steps": [
      ...
      "Use Resend invite for a contact whose invitation needs to be issued again.",
  ...
  },
  {
    ...
    "answer": "The invitation flow uses an account password rather than a permanent passwordless link. The client opens the invitation sent to their email address, accepts it, completes the email-verification step when requested, and sets a password. Subsequent client activity—including invitation acceptance, password creation, and portal login—can appear in Recent activity for operational confirmation.",
    "more": [
      "If the client reports that the invitation link does not work, confirm the contact email and resend the invitation from Info. The client should use the newest email, complete the verification code step, set a password, and then use those credentials for later portal or mobile-app access."
    ],
    "search": "The invitation flow uses an account password rather than a permanent passwordless link. The client opens the invitation sent to their email address, accepts it, completes the email-verification step when requested, and sets a password. Subsequent client activity—including invitation acceptance, password creation, and portal login—can appear in Recent activity for operational confirmation. If the client reports that the invitation link does not work, confirm the contact email and resend the invitation from Info. The client should use the newest email, complete the verification code step, set a password, and then use those credentials for later portal or mobile-app access.",
    "read": "2 min",
    "path": "assurepro/index.html"
  },
  {
  ...
  },
  {
    ...
    "answer": "Open Prospects and select New prospect. Enter the required name and email, then add any company, deal value, phone, stage, source, owners, expected close date, tags, lead type, referrer, industry, size, job role, designation, service interest, and linked contact. Select Add prospect to place the lead in the chosen pipeline stage.",
    "more": [
      "After saving, confirm that the lead appears in the intended pipeline and stage and that the owner and expected close date are correct. Add a task or note for the next action; when the opportunity succeeds, move it to Won so the lead becomes a client without losing the prospect history."
    ],
    "search": "Open Prospects and select New prospect. Enter the required name and email, then add any company, deal value, phone, stage, source, owners, expected close date, tags, lead type, referrer, industry, size, job role, designation, service interest, and linked contact. Select Add prospect to place the lead in the chosen pipeline stage. After saving, confirm that the lead appears in the intended pipeline and stage and that the owner and expected close date are correct. Add a task or note for the next action; when the opportunity succeeds, move it to Won so the lead becomes a client without losing the prospect history.",
    "read": "2 min",
    "path": "assurepro/index.html"
  },
  {
  ...
  },
  {
    ...
    "answer": "Open Settings, choose Documents, and stay on Folder Templates. Select New template to create a structure or open an existing template, then use Add Folder to build its hierarchy. The finished template can be applied from a client's Documents tab or selected as an onboarding default.",
    "more": [
      "Design the template around a repeatable service or period rather than one exceptional client. After editing, test Apply Template in a demo client to confirm folder names and hierarchy; then select the template in Onboarding Defaults if it should be used automatically for future clients."
    ],
    "search": "Open Settings, choose Documents, and stay on Folder Templates. Select New template to create a structure or open an existing template, then use Add Folder to build its hierarchy. The finished template can be applied from a client's Documents tab or selected as an onboarding default. Design the template around a repeatable service or period rather than one exceptional client. After editing, test Apply Template in a demo client to confirm folder names and hierarchy; then select the template in Onboarding Defaults if it should be used automatically for future clients.",
    "read": "2 min",
    "path": "assurepro/index.html"
  },
  {
  ...
  },
  {
    ...
    "steps": [
      ...
      "Review or edit the initial delay, repeat interval, and total number of reminders.",
